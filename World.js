...
World.prototype = {
	...
	columnRow: function (radius, height, number, dx) {
		...
		for (var i=0; i < number; i++) {
			column = new THREE.Mesh(
				new THREE.CylinderGeometry(radius, radius, height, 12, 4, false),
	...
	palace: function () {
		...
		roof = new THREE.Mesh(
			new THREE.CubeGeometry(17, 11, 0.5),
			new THREE.MeshLambertMaterial({
				ambient: 0xCCEECC,
				color: 0xCCEECC
			})
	...
	church: function () {
		...
		towerRoof = new THREE.Mesh(
			new THREE.CylinderGeometry(r,2.12,3,4,4,false),
			new THREE.MeshLambertMaterial({
				ambient: 0xFFAA99,
				color: 0xFFAA99,
		...
		crossVerticalBar = new THREE.Mesh(
			new THREE.CubeGeometry(r*1.41, r*1.41, 4),
			new THREE.MeshLambertMaterial({
				ambient: 0xCACA9C,
				color: 0xCACA9C
			})
		...
		crossHorizontalBar = new THREE.Mesh(
			new THREE.CubeGeometry(r*1.41, r*1.41, 2),
			new THREE.MeshLambertMaterial({
				ambient: 0xCACA9C,
				color: 0xCACA9C
			})
	...
	runway: function () {
		...
		runway = new THREE.Mesh(
			new THREE.PlaneGeometry(10, 100, 2, 8),
			new THREE.MeshLambertMaterial({
				ambient: 0xAAAAAA,
				color: 0x777777
			})
